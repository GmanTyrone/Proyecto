...
print(
  alphaobj %>%
    ...
    ggplot() +
    geom_boxplot(aes(y = `Alpha Diversity Measure`, fill = alcohol_frequent), outlier.shape = 21, width = 5) +
    ...
    scale_fill_manual("alcohol_frequent", values=c("#E69F00", "#0072B2"))
)
...
for(i in seq_along(metric)) {
  alpha_diversity[1,i] <- coin::wilcox_test(reformulate("factor(alcohol_frequent)", metric[[i]]), data = alphaobj) %>% pvalue()
}
...
print(
  ggplot(beta_canberra, aes(x = Axis.1, y = Axis.2, color = alcohol_frequent)) +
    geom_point() +
    stat_ellipse(type = "t") +
    scale_colour_manual("alcohol_frequent", values=c("#E69F00", "#0072B2")) +
...
print(
  ggplot(beta_unweighted_unifrac, aes(x = Axis.1, y = Axis.2, color = alcohol_frequent)) +
    geom_point() +
    stat_ellipse(type = "t") +
    scale_colour_manual("alcohol_frequent", values=c("#E69F00", "#0072B2")) +
...
print(
  ggplot(beta_weighted_unifrac, aes(x = Axis.1, y = Axis.2, color = alcohol_frequent)) +
    geom_point() +
    stat_ellipse(type = "t") +
    scale_colour_manual("alcohol_frequent", values=c("#E69F00", "#0072B2")) +
...
beta_diversity <- data.frame(metric = metric, p.value = NA)
...
for (i in seq_along(metric)) {
  ...
  sampleda$alcohol_frequent <- factor(sampleda$alcohol_frequent)
  
  adonis_result <- adonis(formula = distme ~ alcohol_frequent, data = sampleda, permutations = 999)
  beta_diversity[i, "p.value"] <- adonis_result$aov.tab[1, "Pr(>F)"]
}
