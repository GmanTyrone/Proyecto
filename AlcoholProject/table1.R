...
# Parkinson_disease
parkinson_disease_results <- data %>%
  group_by(parkinson_disease, alcohol_frequent) %>%
  summarise(mean = mean(age, na.rm = TRUE), .groups = 'drop') %>%
  pivot_wider(names_from = alcohol_frequent, values_from = mean)
p_value <- chisq.test(table(data$parkinson_disease, data$alcohol_frequent))$p.value
parkinson_disease_results <- bind_rows(parkinson_disease_results, c("p_value" = p_value))
print(parkinson_disease_results)
