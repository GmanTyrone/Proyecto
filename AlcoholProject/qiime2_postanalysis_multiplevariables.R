...
sample <- "phyloseq/hillburns_metadata_Clean.tsv"
...
# Define the order of alcohol_how_much categories
alcohol_order <- c("None", "Less than 2 drinks a week", "1 drink a day", "2 drinks a day", "2-6 drinks a week", "3 or more drinks a day")
...
print(
  alphaobj %>%
    ...
    ggplot() +
    geom_boxplot(aes(y = `Alpha Diversity Measure`, fill = factor(alcohol_how_much, levels = alcohol_order)), outlier.shape = 21, width = 5) +
    ...
    scale_fill_manual("alcohol_how_much", values=c("#E69F00", "#0072B2", "#D55E00", "#56B4E9", "#009E73", "#F0E442"))
)
...
for(i in seq_along(metric)) {
  alpha_diversity[1,i] <- kruskal.test(as.formula(paste(metric[[i]], "~ alcohol_how_much")), data = alphaobj)$p.value
}
colnames(alpha_diversity) <- metric
row.names(alpha_diversity) <- "p.value"
...
print(
  ggplot(beta_canberra, aes(x = Axis.1, y = Axis.2, color = factor(alcohol_how_much, levels = alcohol_order))) +
    geom_point() +
    stat_ellipse(type = "t") +
    scale_colour_manual("alcohol_how_much", values=c("#E69F00", "#0072B2", "#D55E00", "#56B4E9", "#009E73", "#F0E442")) +
...
# beta diversity plot: unweighted Unifrac
...
print(
  ggplot(beta_unweighted_unifrac, aes(x = Axis.1, y = Axis.2, color = factor(alcohol_how_much, levels = alcohol_order))) +
    geom_point() +
    stat_ellipse(type = "t") +
    scale_colour_manual("alcohol_how_much", values=c("#E69F00", "#0072B2", "#D55E00", "#56B4E9", "#009E73", "#F0E442")) +
...
print(
  ggplot(beta_weighted_unifrac, aes(x = Axis.1, y = Axis.2, color = factor(alcohol_how_much, levels = alcohol_order))) +
    geom_point() +
    stat_ellipse(type = "t") +
    scale_colour_manual("alcohol_how_much", values=c("#E69F00", "#0072B2", "#D55E00", "#56B4E9", "#009E73", "#F0E442")) +
...
for(i in seq_along(metric)) {
  ...
  sampleda$alcohol_how_much <- factor(sampleda$alcohol_how_much, levels = alcohol_order)
  set.seed(1234)
  beta_diversity
}
