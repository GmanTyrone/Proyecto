...
data_table1[gsub("`", "", compared_group)] <- factor(data_table1 %>% pull(eval(parse(text = compared_group))),levels = c(compared_levels,Inf),labels = c(compared_labels,"P-value"))
...
# Export Table 1 to a CSV file
write.csv(tb1, "table1_output.csv", row.names = FALSE)
